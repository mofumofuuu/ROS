...
1.写入串口
（1）内容：左右轮速度，单位为mm/s
（2）格式：4 字节[0xff][右轮速度1字节][左轮速度1字节] [0xfe]
2.读取串口
...
int linear_temp1=0;// 暂存的线速度值
/****************************************************/
unsigned char data_terminal_head=0xff;
unsigned char data_terminal_rear=0xfe;
unsigned char data_terminal0=0x0d;  //“/r"字符
unsigned char data_terminal1=0x0a;  //“/n"字符
unsigned char speed_data[4]={0};   //要发给串口的数据
...
void callback(const geometry_msgs::Twist & cmd_input)//订阅/cmd_vel主题回调函数
{
    ...
    linear_temp1 = (int)(linear_temp );
    
    //将转换好的小车速度分量为左右轮速度
    // left_speed_data.d = linear_temp - 0.5f*angular_temp*D ;
    // right_speed_data.d = linear_temp + 0.5f*angular_temp*D ;
    left_speed_data.d = linear_temp1 - 0.5f*angular_temp*D ;
    right_speed_data.d = linear_temp1 + 0.5f*angular_temp*D ;
    //存入数据到要发布的左右轮速度消息
    ...
    // }
     speed_data[1]=  right_speed_data.data[1];
     speed_data[2]=left_speed_data.data[2];
 
    //在写入串口的左右轮速度数据后加入”/r/n“
    // speed_data[8]=data_terminal0;
    // speed_data[9]=data_terminal1;
    speed_data[0]=data_terminal_head;
    speed_data[3]=data_terminal_rear;
    //写入数据到串口
    // my_serial.write(speed_data,10);
 
      ROS_INFO("Sending to serial: Left wheel speed = %d mm/s, Right wheel speed = %d mm/s",
             speed_data[2], speed_data[1]);
    my_serial.write(speed_data,4);
}
